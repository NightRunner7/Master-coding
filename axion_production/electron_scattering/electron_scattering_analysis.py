"""
Electron Scattering Axion Distribution and ΔN_eff Computation
-------------------------------------------------------------

This module provides tools for analyzing the axion distribution function
resulting from **electron scattering production**. It builds upon the interpolated
...
2. **Fitting Function Integration**
   - Uses `evaluate_parameter()` from `electron_scattering` to obtain best-fit
...
electron_scattering = ElectronScatteringModel("data/axion_distributions.dat")

# Generate interpolated distribution
q_values = np.linspace(0.01, 20, 100)
axion_mass = 1e-3  # eV
distribution = electron_scattering.generate_second_interpolation(axion_mass, q_values)

# Compute ΔN_eff for a given axion mass
delta_N_eff = electron_scattering.calculate_delta_n_eff(axion_mass)
"""
...
class ElectronScatteringModel(FirstInterpolation):
    """
    Class for handling axion distribution functions from electron scattering production.
    ...
    def __init__(self, file_path):
        """
        Initialize axion model class for electron scattering.
        ...
        self.con["electron_mass"] = 510.998*10**3  # [eV]
        ...
        self.con["g_star_s_axion_decoupling"] = RelativisticDOF.compute_decoupling_dof(self.con["electron_mass"], self.con["x_dec"])
