"""
Muon Scattering Axion Distribution and ΔN_eff Computation
----------------------------------------------------------

This module provides tools for analyzing the axion distribution function
resulting from **muon scattering production**. It builds upon the interpolated
...
2. **Fitting Function Integration**
   - Uses `evaluate_parameter()` from `muon_scattering` to obtain best-fit
...
muon_scattering = MuonScatteringModel("data/axion_distributions.dat")

# Generate interpolated distribution
q_values = np.linspace(0.01, 20, 100)
axion_mass = 1e-3  # eV
distribution = muon_scattering.generate_second_interpolation(axion_mass, q_values)

# Compute ΔN_eff for a given axion mass
delta_N_eff = muon_scattering.calculate_delta_n_eff(axion_mass)
"""
...
class MuonScatteringModel(FirstInterpolation):
    """
    Class for handling axion distribution functions from muon scattering production.
    ...
    def __init__(self, file_path):
        """
        Initialize axion model class for muon scattering.
        ...
        self.con["muon_mass"] = 105.66*10**6  # [eV]
        self.con["kB_T_today"] = 8.617 * 10**(-5) * 2.725  # kB * T, [kB*T] = [kB] * [T] = [eV/K] * [K] = [eV]
        self.con["x_dec"] = 30  # Axion decoupling scale, 20
        ...
        self.con["g_star_s_axion_decoupling"] = RelativisticDOF.compute_decoupling_dof(self.con["muon_mass"], self.con["x_dec"])
    ...
    def calculate_delta_n_eff(self, m_a, parameters_arr=None):
        ...
        # --- Deal with Rescaled Distribution
        ...
        # def distribution_tilda(q_tilda):
        #     """
        ...
        # integral_of_dist, _ = quad(distribution_tilda, 0, q_tilda_upper_limit)
        integral_of_dist, _ = quad(distribution_tilda, 0, self.con["q_max"])
